...
exports.add = function(db) {
	return function(req, res){
		//找到最大id 
		table.findOne({}, {sort: {id: -1}}).then((obj) => {
			let hero = req.body;
			hero.id = (parseInt(obj.id)+1)+"";
			//在数据库里插入新对象，并返回添加成功的对象
			table.insert(hero,function(){
				console.log('新增英雄成功');
			}).then((result) => res.json(result)).then(() => db.close());   
...
exports.update = function(db){
	return function(req, res){
		...
		let hero = req.body;
		table.update({'id': hero.id}, hero).then((result) => res.json(result)).then(() => db.close());
	}
}
